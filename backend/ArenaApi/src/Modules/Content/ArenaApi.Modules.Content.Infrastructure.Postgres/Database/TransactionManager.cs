...
namespace ArenaApi.Modules.Content.Infrastructure.Postgres.Database;

/// TransactionManager backed by Wolverine durable outbox.
/// Auto-mode: SaveChangesAsync without prior BeginTransactionAsync uses
/// SaveChangesAndFlushMessagesAsync (atomic DB save + outbox dispatch).
/// Manual-mode: Begin -> 1+ Save -> Commit. Rollback on Dispose if not committed.
///
/// Phase 0: domain-event dispatching is deferred — aggregates raise events
/// into their internal list, but no in-process handlers exist yet.
internal sealed class TransactionManager(
    IDbContextOutbox<ContentDbContext> outbox,
